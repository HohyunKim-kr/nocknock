...
++  set-mining
  |=  mine=?
  ^-  mining-state:dk
  ?:  &(mine =(*(z-set lock:t) pubkeys.m))
    ~>  %slog.[0 [%leaf "Cannot start mining without pubkey set"]]
    m
  ?:  &(mine =(*(z-map lock:t @) shares.m))
    ~>  %slog.[0 [%leaf "Cannot start mining without shares set"]]
    m
  m(mining mine)
::
...
++  set-pubkeys
  ...
  ^-  mining-state:dk
  ?:  =(0 (lent pks))
    ~>  %slog.[0 [%leaf "Cannot set empty pubkey list"]]
    m
  =.  pubkeys.m
